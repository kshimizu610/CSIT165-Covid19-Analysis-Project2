...
leaflet(global_joined) %>% 
  ...
  hideGroup("deaths")
```
### Objective 2: Table with Top 25 Countries by Confirmations and Deaths
```{r ob2}
# Identify top 25 countries for confirmations
confirmed_top <- global_joined %>% 
  arrange(desc(confirmed)) %>% 
  slice(1:25) %>% 
  select(Country = `Country.Region`, Count = confirmed)

# Identify top 25 countries for deaths
deaths_top <- global_joined %>% 
  arrange(desc(deaths)) %>% 
  slice(1:25) %>% 
...
table_ranked <- tibble(
  Rank = 1:25,
  country_confirmed = confirmed_top$Country,
  country_count = confirmed_top$Count,
  country_deaths = deaths_top$Country,
  count_deaths = deaths_top$Count
)
